use crate::{
    BookState, ConfigWire, EngineMetadata, EventFrame, FinishFrame, FrameType, HelloFrame,
    ObservationFrame, PROTOCOL_NAME, PROTOCOL_VERSION, ReadyFrame, SnapshotRequest, write_frame,
};
use serde::Serialize;
...
pub trait EngineAdapter {
    fn apply(&mut self, event: &crate::MarketEvent, index: u64)
...
pub fn run<A, F>(identity: EngineIdentity, factory: F) -> i32
where
    A: EngineAdapter,
    F: FnOnce(ConfigWire) -> Result<A, String>,
{
    let stdin = io::stdin();
    ...
    match serve(&mut input, &mut output, identity, factory) {
        Ok(()) => 0,
    ...
    F: FnOnce(ConfigWire) -> Result<A, String>,
{
    ...
    let mut adapter = factory(hello.config).map_err(ServerError::Protocol)?;
    send(
        output,
        &ReadyFrame {
            kind: "ready",
    ...
    loop {
        ...
        match frame_type.kind.as_str() {
            "event" => {
                ...
                if frame.index == 0 || frame.index != last_index + 1 {
...
mod tests {
    ...
    impl EngineAdapter for EmptyAdapter {
        fn apply(
            &mut self,
            _event: &crate::MarketEvent,
            _index: u64,
        ) -> Result<ObservationFrame, String> {
            Err("empty adapter does not accept events".to_string())
        }

        fn snapshot(&self) -> Result<BookState, String> {
            Ok(BookState { books: Vec::new() })
        }
    }
    ...
    fn empty_session_completes_with_protocol_defaults() {
        ...
        assert_eq!(values[0]["engine"]["version"], "0.1.0");
