...
from tracebook.simulation.simulation_engine import SimulationConfig


def run_basic_profiling_check():
...
def run_order_book_profiling_check():
    ...
    # Start profiling session
    with profiler.profile_session("orderbook_test"):
        ...
        for i in range(100):
            # Create buy order
            buy_order = factory.create_limit_order("BTCUSD", OrderSide.BUY, 50000.0 + i, 100)
            order_book.add_order(buy_order)

            # Create sell order
            sell_order = factory.create_limit_order("BTCUSD", OrderSide.SELL, 50001.0 - i, 100)
        ...
        print(
            f"✓ Final stats: {final_stats.get('orders_added', 0)} orders, "
...
def run_simulation_profiling_check():
    ...
    trace_config.profile_functions.extend(
        ["generate_order", "process_orders_batch", "run_simulation"]
    )
...
def main():
    ...
    tests = [
        ("Basic Profiling", run_basic_profiling_check),
        ("Order Book Profiling", run_order_book_profiling_check),
        ("Simulation Profiling", run_simulation_profiling_check),
        ("Trace Export", run_trace_export_check),
    ]
