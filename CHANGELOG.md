...
- Established a mypy type-check baseline on the `core` package and enforce it in CI (`mypy src/tracebook/core`, clean). Added the annotations to get there, guarded `infer_price_decimals` against non-int `Decimal` exponents, and restored `mypy` as a dev dependency. Broader module coverage is incremental follow-up.
