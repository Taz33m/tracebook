...
@jit(nopython=True, cache=True)
def fifo_batch_match(
    buy_prices: np.ndarray,
    buy_quantities: np.ndarray,
    buy_timestamps: np.ndarray,
    sell_prices: np.ndarray,
    sell_quantities: np.ndarray,
    sell_timestamps: np.ndarray,
    ...
    Returns:
        Tuple[np.ndarray, np.ndarray, np.ndarray]: execution prices, quantities, and flags
    """
    n_matches = min(len(buy_prices), len(sell_prices))
    execution_prices = np.zeros(n_matches, dtype=np.float64)
    execution_quantities = np.zeros(n_matches, dtype=np.float64)
...
class FIFOAnalyzer:
    ...
    def _calculate_fairness_score(self) -> float:
        ...
        violation_rate = self.fairness_metrics["price_time_priority_violations"] / max(
            1, self.fairness_metrics["total_matches"]
        )
    ...
    def _generate_recommendations(
        ...
        if queue_stats.get("mean_queue_time_ns", 0) > 1_000_000:  # > 1ms
            recommendations.append(
                "Consider optimizing order matching speed - high queue times detected"
            )

        # Analyze fairness
        if self.fairness_metrics["price_time_priority_violations"] > 0:
            recommendations.append(
                "Price-time priority violations detected - review matching logic"
            )

        # Analyze throughput
        if performance_metrics.get("throughput", 0) < 1000:
            recommendations.append(
                "Low throughput detected - consider batch processing optimization"
            )
