...
def calculate_pro_rata_allocation(
    ...
    Returns:
        float: Allocated quantity
    """
    if order_quantity <= 0 or total_quantity <= 0 or available_quantity <= 0:
        return 0.0

    # Calculate proportional allocation
    allocation_ratio = order_quantity / total_quantity
    raw_allocation = available_quantity * allocation_ratio

    # Ensure minimum allocation if possible
    if min_allocation > 0.0 and raw_allocation < min_allocation:
        raw_allocation = min_allocation

    return min(order_quantity, available_quantity, max(0.0, raw_allocation))


@jit(nopython=True, cache=True)
def pro_rata_batch_allocation(
    order_quantities: np.ndarray, available_quantity: float, min_allocation: float = 0.0
) -> np.ndarray:
    """
    ...
    allocations = np.zeros(n_orders, dtype=np.float64)
    ...
    remaining_quantity = float(available_quantity)
    ...
    for i in range(n_orders):
        ...
        # Don't exceed remaining quantity or the order's requested quantity.
        allocation = min(order_quantities[i], min(allocation, remaining_quantity))
    ...
    if remaining_quantity > 1e-12:
        ...
        for idx in sorted_indices:
            ...
            capacity = order_quantities[idx] - allocations[idx]
            if capacity > 0:
                additional_allocation = min(capacity, remaining_quantity)
                allocations[idx] += additional_allocation
                remaining_quantity -= additional_allocation

    return allocations


@jit(nopython=True, cache=True)
def pro_rata_match_single(
    buy_price: float, buy_qty: float, sell_price: float, sell_qty: float, total_sell_qty: float
    ...
    Returns:
        Tuple[float, float, bool]: (execution_price, execution_quantity, can_match)
    """
    ...
    if total_sell_qty > 0:
        ...
        execution_quantity = max_fill * allocation_ratio
    else:
...
class ProRataAnalyzer:
    ...
    def record_allocation(
        ...
        allocation_record = {
            ...
            "fill_rate": total_allocated / max(1e-12, available_quantity),
            "demand_ratio": total_requested / max(1e-12, available_quantity),
            "order_details": [],
        }
        ...
        for order, allocation in zip(orders, allocations):
            ...
            fill_rate = allocation / max(1e-12, order_size)
    ...
    def get_allocation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "order_size_correlation": (
                float(np.corrcoef(order_sizes, individual_fill_rates)[0, 1])
    ...
    def get_fairness_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self._generate_fairness_recommendations(allocation_stats, size_bias),
        }
    ...
    def _categorize_order_size(self, size: float) -> str:
    ...
    def _update_size_bias_analysis(self, order_size: float, fill_rate: float):
    ...
    def _update_fairness_metrics(self, allocation_record: Dict):
        ...
        small_orders = [
            detail for detail in order_details if detail["size_category"] == "small_orders"
        ]
        large_orders = [
            detail for detail in order_details if detail["size_category"] == "large_orders"
        ]
    ...
    def _generate_fairness_recommendations(
        self, allocation_stats: Dict, size_bias: Dict
    ) -> List[str]:
        """Generate allocation fairness recommendations."""
        recommendations = []
        ...
        if allocation_stats and "individual_order_stats" in allocation_stats:
            ...
            if variance > 0.1:  # High variance threshold
                recommendations.append(
                    "High allocation variance detected. "
                    "Consider implementing minimum allocation rules."
                )
        ...
        if fairness_score < 70:
            recommendations.append("Low fairness score. Consider hybrid allocation mechanisms.")
    ...
    def analyze_performance(
        self, order_book_stats: Dict[str, Any], performance_metrics: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                order_book_stats, performance_metrics
            ),
        }
    ...
    def _generate_recommendations(
        ...
        if self.fairness_metrics["allocation_variance"] > 0.5:
            recommendations.append(
                "High allocation variance detected - consider minimum allocation thresholds"
            )

        # Analyze size bias
        if self.fairness_metrics["small_order_penalty"] > 0.2:
            recommendations.append(
                "Small orders experiencing significant penalty - review allocation algorithm"
            )

        # Analyze throughput
        if performance_metrics.get("throughput", 0) < 500:
            recommendations.append(
                "Low throughput for Pro-Rata - consider optimizing allocation calculations"
            )
