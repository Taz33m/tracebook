...
class TraceVisualizer:
        ...
        for _, row in df.iterrows():
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    line=dict(color=color_map[row["Function"]], width=8),
                    name=row["Function"],
                    showlegend=False,
                    hovertemplate=(
    ...
    def create_function_performance_chart(self) -> go.Figure:
        ...
        # Mean duration
        fig.add_trace(
            go.Bar(x=functions, y=mean_durations, name="Mean Duration (ms)"), row=1, col=1
        )

        # Total time
        fig.add_trace(go.Bar(x=functions, y=total_times, name="Total Time (ms)"), row=1, col=2)

        # Call count
        fig.add_trace(go.Bar(x=functions, y=call_counts, name="Call Count"), row=2, col=1)

        # P99 latency
        fig.add_trace(go.Bar(x=functions, y=p99_durations, name="P99 Latency (ms)"), row=2, col=2)

        fig.update_layout(title="Function Performance Analysis", height=800, showlegend=False)
    ...
    def create_latency_distribution(self) -> go.Figure:
        ...
        for func_name, latencies in function_latencies.items():
            fig.add_trace(go.Histogram(x=latencies, name=func_name, opacity=0.7, nbinsx=30))
    ...
    def create_call_depth_analysis(self) -> go.Figure:
        ...
        for func_name in all_functions:
            ...
            fig.add_trace(
                go.Bar(x=depths, y=counts, name=func_name, marker_color=color_map[func_name])
            )
    ...
    def create_insights_report(self) -> str:
        ...
        if "function_analysis" in self.trace_data:
            ...
            sorted_by_avg = sorted(
                analysis.items(), key=lambda x: x[1]["mean_duration_ms"], reverse=True
            )
