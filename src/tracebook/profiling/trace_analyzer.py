...
class HighResolutionTracer:
    ...
    def trace_function_enter(self, function_name: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        with self.lock:
            ...
            self.metrics["max_call_depth"] = max(self.metrics["max_call_depth"], call_depth + 1)
    ...
    def trace_function_exit(self, function_name: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        with self.lock:
            ...
            if thread_id in self.active_calls and function_name in self.active_calls[thread_id]:
    ...
    def _analyze_trace_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "call_depth_distribution": {
                ...
                "distribution": (
                    {int(k): int(v) for k, v in zip(*np.unique(call_depths, return_counts=True))}
                    if call_depths
                    else {}
                ),
            },
    ...
    def _generate_performance_insights(self, function_analysis: Dict[str, Any]) -> List[str]:
        ...
        sorted_functions = sorted(
            function_analysis.items(), key=lambda x: x[1]["mean_duration_ns"], reverse=True
        )
        ...
        high_variance_funcs = [
            (name, stats)
            for name, stats in function_analysis.items()
        ...
        sorted_by_total = sorted(
            function_analysis.items(), key=lambda x: x[1]["total_time_ns"], reverse=True
        )
