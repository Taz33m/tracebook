...
class MagicTraceProfiler:
    ...
    def get_active_sessions(self) -> List[str]:
        ...
        with self._lock:
            return [name for name, session in self.active_sessions.items() if session.is_active]
                ...
                try:
                    ...
                    with self._lock:
                        ...
                        if len(self.metrics["function_latencies"][name]) > 1000:
                            self.metrics["function_latencies"][name] = self.metrics[
                                "function_latencies"
                            ][name][-1000:]
    ...
    def _trigger_latency_alert(self, func_name: str, latency_ns: int):
        ...
        print(
            f"LATENCY ALERT: {func_name} took {latency_ns/1_000_000:.2f}ms "
        ...
        for func_name, stats in func_perf.items():
            ...
            if p99_latency > 10_000_000:  # > 10ms
                recommendations.append(
                    f"Function '{func_name}' has high P99 latency "
                    f"({p99_latency/1_000_000:.1f}ms). "
                    "Consider optimization or async processing."
                )

            if mean_latency > 1_000_000:  # > 1ms
                recommendations.append(
                    f"Function '{func_name}' has high average latency "
                    f"({mean_latency/1_000_000:.1f}ms). "
