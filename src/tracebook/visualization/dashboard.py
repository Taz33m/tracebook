...
class PerformanceDashboard:
        ...
        app.layout = html.Div(
            [
                ...
                html.Div(
                    [
                        html.H1(
                            "Order Book Performance Dashboard",
    ...
    def _register_callbacks(self, app: dash.Dash):
        ...
        @app.callback(
            Output("throughput-chart", "figure"), [Input("interval-component", "n_intervals")]
        )
        ...
        @app.callback(
            Output("latency-chart", "figure"), [Input("interval-component", "n_intervals")]
        )
        ...
        @app.callback(
            Output("system-resources-chart", "figure"), [Input("interval-component", "n_intervals")]
        )
        ...
        @app.callback(
            Output("order-book-depth-chart", "figure"), [Input("interval-component", "n_intervals")]
        )
                    ...
                    if order_books:
                        ...
                        ob_data = {
                            "bids": [
                                ...
                                for price, quantity, _ in snapshot.bid_levels
                            ],
                            "asks": [
                                {"price": price, "quantity": quantity}
                                for price, quantity, _ in snapshot.ask_levels
                            ],
                        }
    ...
    def run(self, debug: bool = False, host: str = "127.0.0.1"):
        ...
        try:
            run_server = getattr(self.app, "run", None) or self.app.run_server
            run_server(debug=debug, host=host, port=self.port)
        finally:
...
def create_dashboard(
    port: int = 8050, update_interval: int = 1000, performance_monitor=None
...
def main() -> int:
    ...
    parser.add_argument(
        "--demo-simulation",
        action="store_true",
        help="Run a background simulation for live dashboard data.",
    )
    parser.add_argument(
        "--demo-duration", type=float, default=60.0, help="Demo simulation duration in seconds."
    )
    parser.add_argument(
        "--demo-throughput",
        type=float,
        default=500.0,
        help="Demo simulation target orders per second.",
    )
    parser.add_argument(
