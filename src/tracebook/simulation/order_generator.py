...
class SimulationEventType(IntEnum):
    """Event types emitted by synthetic simulations."""
...
class SimulationEvent:
    """Canonical event emitted by synthetic order streams."""
...
class PriceModel:
    ...
    def update_price(self, dt_seconds: float = 0.001) -> float:
        ...
        if self.deterministic:
            current_time = self.last_update_time + int(dt_seconds * 1_000_000_000)
            dt = dt_seconds
        else:
        ...
        random_component = self.volatility * self.current_price * self.rng.normal(0, np.sqrt(dt))

        # Jump component
        jump_component = 0.0
        if self.rng.random() < self.jump_intensity * dt:
            jump_size = self.rng.normal(0, self.jump_size_std)
    ...
    def get_price_levels(
        self, num_levels: int = 5, spread_multiplier: float = 1.0
    ) -> Dict[str, List[float]]:
...
class RandomOrderGenerator(OrderGenerator):
    ...
    def generate_orders(self, count: int) -> List[Order]:
        ...
        for _ in range(count):
            ...
            is_market_order = self.rng.random() < self.market_params.market_order_ratio
            ...
            quantity = self.rng.uniform(
                self.market_params.min_quantity, self.market_params.max_quantity
            )
            ...
            if order_type == OrderType.MARKET:
                ...
                if side == OrderSide.BUY:
                    ...
                    price = self.rng.uniform(bid_price - price_range, bid_price + price_range * 0.2)
                else:
                    # Sell limit order: price around or above current ask
                    price_range = ask_price * 0.05  # 5% range
                    price = self.rng.uniform(ask_price - price_range * 0.2, ask_price + price_range)
...
class MarketMakingOrderGenerator(OrderGenerator):
    ...
    def generate_orders(self, count: int) -> List[Order]:
        ...
        # Get price levels
        price_levels = self.price_model.get_price_levels(self.num_levels, self.spread_multiplier)

        orders_per_side = count // 2

        # Generate bid orders
        for i in range(orders_per_side):
            price = price_levels["bids"][i % len(price_levels["bids"])]
            quantity = self.rng.uniform(
        ...
        # Generate ask orders
        for i in range(count - len(orders)):
            price = price_levels["asks"][i % len(price_levels["asks"])]
            quantity = self.rng.uniform(
                self.market_params.min_quantity * self.quantity_multiplier,
                self.market_params.max_quantity * self.quantity_multiplier,
            )
...
class AggressiveOrderGenerator(OrderGenerator):
    ...
    def generate_orders(self, count: int) -> List[Order]:
        ...
        for _ in range(count):
            ...
            # Determine order type
            is_market_order = self.rng.random() < self.market_order_ratio
            ...
            quantity = self.rng.uniform(
                self.market_params.min_quantity * 2, self.market_params.max_quantity * 1.5
            )
            ...
            if is_market_order:
                ...
                if side == OrderSide.BUY:
                    ...
                    if self.rng.random() < self.aggressive_limit_ratio:
                        price = ask_price + self.rng.uniform(0, ask_price * 0.01)
                    ...
                    if self.rng.random() < self.aggressive_limit_ratio:
                        price = bid_price - self.rng.uniform(0, bid_price * 0.01)
                        self.aggressive_limit_orders += 1
                    else:
                        price = ask_price
...
class SyntheticOrderStream:
    ...
    def __init__(
        ...
        for generator in self.generators:
            generator.order_factory = self.order_factory
    ...
    def _create_generators(self) -> List[OrderGenerator]:
        ...
        if self.config.pattern == OrderPattern.RANDOM:
            generators.append(RandomOrderGenerator(self.market_params, self._generator_config(0)))

        elif self.config.pattern == OrderPattern.MARKET_MAKING:
            generators.append(
                MarketMakingOrderGenerator(self.market_params, self._generator_config(0))
            )

        elif self.config.pattern == OrderPattern.AGGRESSIVE:
            generators.append(
                AggressiveOrderGenerator(self.market_params, self._generator_config(0))
            )

        elif self.config.pattern == OrderPattern.TREND_FOLLOWING:
            generators.append(
                RandomOrderGenerator(
                    replace(self.market_params, trend_strength=0.7),
                    self._generator_config(0),
                )
            )

        elif self.config.pattern == OrderPattern.MEAN_REVERTING:
            generators.append(
                RandomOrderGenerator(
                    replace(self.market_params, mean_reversion_speed=0.5, trend_strength=0.0),
                    self._generator_config(0),
                )
            )

        elif self.config.pattern == OrderPattern.MOMENTUM:
            generators.append(
                RandomOrderGenerator(
                    replace(
                        self.market_params,
                        trend_strength=1.0,
                        price_volatility=self.market_params.price_volatility * 1.5,
                    ),
                    self._generator_config(0),
                )
            )

        elif self.config.pattern == OrderPattern.PASSIVE:
            generators.append(
                RandomOrderGenerator(
                    replace(self.market_params, market_order_ratio=0.05),
                    self._generator_config(0),
                )
            )
            ...
            generators.extend(
                [
                    RandomOrderGenerator(self.market_params, self._generator_config(0)),
                    MarketMakingOrderGenerator(self.market_params, self._generator_config(1)),
                    AggressiveOrderGenerator(self.market_params, self._generator_config(2)),
                ]
            )
    ...
    def _generator_config(self, offset: int) -> OrderGenerationConfig:
        """Return a generator-specific config with an independent deterministic seed."""
        if self.config.seed is None:
            return self.config
        return replace(self.config, seed=self.config.seed + offset)
    ...
    def get_orders(self, max_count: Optional[int] = None) -> List[Order]:
        ...
        with self.queue_lock:
            if max_count is None:
                ...
                self.events_queue = self.events_queue[len(orders) :]
    ...
    def get_events(self, max_count: Optional[int] = None) -> List[SimulationEvent]:
        ...
        with self.queue_lock:
            if max_count is None:
                ...
                events = self.events_queue[:max_count]
                self.events_queue = self.events_queue[max_count:]
                self.orders_queue = self.orders_queue[len(events) :]
    ...
    def get_stream_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "market_params": {
                "symbol": self.market_params.symbol,
                "current_price": (
                    self.generators[0].price_model.current_price if self.generators else 0
                ),
    ...
    def _generation_loop(self):
        ...
        while self.is_running:
            ...
            try:
                ...
                # Add to queue
                new_events = [
                    SimulationEvent(SimulationEventType.NEW, self.market_params.symbol, order=order)
                    for order in new_orders
                ]
                with self.queue_lock:
                    self.orders_queue.extend(new_orders)
                    self.events_queue.extend(new_events)
...
def create_order_stream(
    ...
    Args:
        ...
        seed: Optional random seed for deterministic order flow

    Returns:
        Configured SyntheticOrderStream instance
    """
    market_params = MarketParameters(
        symbol=symbol, initial_price=initial_price, order_arrival_rate=throughput
    )
